...
// Step 1: parameters required before running the script:
// 'stDate': local time of ignition
// 'ignition': longitude, latitude of ignition
// 'state': state where the fire started
// 'official': acres of the final fire

// Step 2: initial guess for other parameters
// 'AOI': footprint of the fire
// make a rectangle that expands from the initial ignition point, +/- 0.2 degrees
// ee.Geometry.Rectangle([MIN LON, MIN LAT, MAX LON, MAX LAT])
// 'tz': input local time zone

// Step 3: compute UTC start time
// run the script in mapMode = false
// 'start': copy the start UTC time from console, in the format Y-MM-dd HH
// 'end': make a guess for the end UTC time, initially for 2 weeks, in the format Y-MM-dd HH

// Step 4: estimate 'AOI' and 'end' parameters
// run the script in mapMode = true
// adjust 'AOI' based on the footprint of the GOES pixels
// adjust 'end' based on the timeseries of the GOES active fire pixel counts
// printed in the console
// when to finalize:
// 1. 'AOI' should encompass both the GOES-East (gray) and GOES-West (black) pixels
// 2. 'end' should encompass the entire lifetime of the fire
// 3. approximate area of the final fire should be in the same order of magnitude
// as 'official' acres
// 4. if this approximate area is too small compared to 'official', try adjusting
// 'AOI' and 'end' parameters

// Step 5: adjust 'nHour'
// 'nHour' should be the x-axis value of the last peak (non-zero count) in the timeseries
// can expand the timeseries in another window and interactively hover over timeseries

// Step 6: adjust 'end'
// run the script again
// 'end': enter the updated end UTC time based on the console
// 'nDay': enter the number of days, rounded to the next integer, based on the console

// metadata dictionary for input fire
var fireDict = {
  ...
  stDate: 'October 23, 2019 9:27 PM',
...
var mapMode = true; // true = print layers on map

// input local time zone and print the UTC time
// https://en.wikipedia.org/wiki/List_of_tz_database_time_zones
// https://upload.wikimedia.org/wikipedia/commons/8/88/World_Time_Zones_Map.png
var tz = 'America/Los_Angeles'; // tz is the TZ indentifier (format is continent/city)
var inDate = ee.Date.parse('MMM d, Y K:m aa',fireDict.stDate,tz);
print('start: ', inDate
  .format('Y-MM-dd HH','UTC'));
print('end: ', fireDict.start.advance(fireDict.nHour,'hour')
  .format('Y-MM-dd HH','UTC'));
print('nDay: ', Math.ceil(fireDict.nHour/24));

var goesEast_col = goferFun.getGOEScol(inDate,'GOES-East');
var goesWest_col = goferFun.getGOEScol(inDate,'GOES-West');
print('GOES-East:', goesEast_col.first());
print('GOES-West:', goesWest_col.first());
...
if (mapMode) {
  ...
  print('approximate area of final fire (acres): ', getFireArea(affected_areas));
