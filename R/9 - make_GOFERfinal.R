...
# make_GOFERfinal.R
# --------------------------------------------
# combine fireProg, fireLine, fireIg, summary
# into a single file for all fires
# --------------------------------------------
...
date2str <- function(x) {format(x,"%Y-%m-%d %H:%M:%S")}
...
for (satMode in c("gofer_combined","gofer_east","gofer_west")) {
    ...
    if (satMode == "gofer_combined") {
      fireSummary_list[[fireDataIdx]] <- fireSummary %>%
        ...
        dplyr::rename(tUTC="time_UTC",tLocal="time_local",tLocalGMT="time_localGMT",timestep="timeStep",
                      ...
                      rfline_fstate="rfline_fst",cfline5_fstate="cfline5_fst",cfline_fstate="cfline10_fst",
        ...
        dplyr::select(fname,fyear,tUTC,tLocal,tLocalGMT,timestep,timestep_hh,
                      ...
                      rfline_fstate,cfline5_fstate,cfline_fstate,
  ...
  output_folder <- paste0(dataFolder,"/",satMode_final)
  ifelse(!file.exists(output_folder),dir.create(output_folder),F)
  
  fireProgShp_list <- do.call(rbind,fireProgShp_list)
  st_write(fireProgShp_list,paste0(dataFolder,"/",satMode_final,"/",satMode_shortName,"_fireProg.shp"),
           quiet=T,append=F)
  
  cfireLine_list <- do.call(rbind,cfireLine_list)
  st_write(cfireLine_list,paste0(dataFolder,"/",satMode_final,"/",satMode_shortName,"_cfireLine.shp"),
           quiet=T,append=F)
  
  rfireLine_list <- do.call(rbind,rfireLine_list)
  st_write(fireProgShp_list,paste0(dataFolder,"/",satMode_final,"/",satMode_shortName,"_rfireLine.shp"),
           quiet=T,append=F)
  ...
  st_write(fireIg_list,paste0(dataFolder,"/",satMode_final,"/",satMode_shortName,"_fireIg.shp"),
           quiet=T,append=F)
